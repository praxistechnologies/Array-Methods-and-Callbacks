...
function getWinners(fifaData, getFinals) {
    ...
    data.map((item) => {    
        if(item["Home Team Goals"] > item["Away Team Goals"]) {    
            newArray.push(item["Home Team Name"])
        }
        if(item["Home Team Goals"] < item["Away Team Goals"]) {
            newArray.push(item["Away Team Name"])
        }
        if(item["Home Team Goals"] == item["Away Team Goals"]) {    
            let winner = item["Win conditions"].split(" ")[0]
...
//.reduce((a, b) => a + b) / data.length).toFixed(2);


function getAverageGoals(fifaData) {
   let avgGames = fifaData.map(ele => ele['Home Team Goals'] + ele['Away Team Goals']);
   return (avgGames.reduce((a, b) => a + b) / avgGames.length).toFixed(2);
}
